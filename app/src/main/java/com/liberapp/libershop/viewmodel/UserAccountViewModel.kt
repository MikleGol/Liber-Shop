...
import com.liberapp.libershop.LiberApplication
...
class UserAccountViewModel @Inject constructor(
    ...
    private val _deleteUser = MutableStateFlow<Resource<Unit>>(Resource.Unspecified())
    val deleteUser = _deleteUser.asStateFlow()
        ...
        viewModelScope.launch {
            try {
                val imageBitmap = MediaStore.Images.Media.getBitmap(
                    getApplication<LiberApplication>().contentResolver,
                    imageUri
                )
    ...
    private fun saveUserInformation(user: User, shouldRetrievedOldImage: Boolean) {
     ...
     fun deleteUserInformation() {
        ...
        documentRef.delete()
            .addOnSuccessListener {
                viewModelScope.launch {
                    _deleteUser.emit(Resource.Success(Unit))
                }
            }
            .addOnFailureListener { exception ->
                viewModelScope.launch {
                    _deleteUser.emit(Resource.Error(exception.message.toString()))
                }
            }
    }
